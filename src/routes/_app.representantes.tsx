...
export const Route = createFileRoute("/_app/representantes")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Cadastros, cidades e desempenho dos representantes Fruta².",
      },
      { property: "og:title", content: "Representantes — Fruta²" },
      {
        property: "og:description",
        content: "Gerencie representantes e acompanhe o desempenho comercial por período.",
      },
...
const repStatus = (invite: Invite): RepStatus =>
  !invite.accepted_at ? "pending" : invite.status === "inactive" ? "inactive" : "active";
const statusLabel: Record<RepStatus, string> = {
  active: "Ativo",
  inactive: "Inativo",
  pending: "Pendente",
};
...
function RepresentantesPage() {
  ...
  const searchPerformance = async () => {
    if (fromDate && toDate && fromDate > toDate)
      return toast.error("A data inicial deve ser anterior à data final");
    ...
    setPerformance(
      invites.map((invite) => ({
        ...invite,
        repStatus: repStatus(invite),
        ...(invite.accepted_user_id ? byRep.get(invite.accepted_user_id) : undefined),
        hours: invite.accepted_user_id ? (byRep.get(invite.accepted_user_id)?.hours ?? 0) : 0,
        itemsSold: invite.accepted_user_id
          ? (byRep.get(invite.accepted_user_id)?.itemsSold ?? 0)
          : 0,
        grossSales: invite.accepted_user_id
          ? (byRep.get(invite.accepted_user_id)?.grossSales ?? 0)
          : 0,
      })),
    );
    setSearched(true);
  ...
  const beginEdit = (invite: Invite) => {
    ...
    setEditStatus(invite.status === "inactive" ? "inactive" : "active");
    setCitiesText(invite.cities.join(", "));
  };
  ...
  const saveRepresentative = async () => {
    ...
    const { error } = await supabase
      .from("rep_invites")
      .update({ name: editName.trim() || null, email: normalizedEmail, status: editStatus, cities })
      .eq("id", editing.id);
    if (error) return toast.error(error.message);
  ...
  const remove = async (invite: Invite) => {
    if (
      !confirm(`Remover o convite de ${invite.email}? Os dados já lançados continuam no sistema.`)
    )
      return;
  ...
  const filteredInvites = invites.filter((invite) => {
    ...
    return (
      ...
      [
        ...
        invite.email,
        invite.cities.join(" "),
        statusLabel[repStatus(invite)],
      ].some((value) => value.toLocaleLowerCase("pt-BR").includes(term))
    );
  });

  const sortedInvites = useSort(
    filteredInvites,
    {
      name: (item) => item.name ?? item.email,
      email: (item) => item.email,
      status: (item) => statusLabel[repStatus(item)],
      cities: (item) => item.cities.join(", "),
    },
    { key: "name", dir: "asc" },
  );
  const sortedPerformance = useSort(
    performance,
    {
      name: (item) => item.name ?? item.email,
      email: (item) => item.email,
      cities: (item) => item.cities.join(", "),
      status: (item) => statusLabel[item.repStatus],
      hours: (item) => item.hours,
      items: (item) => item.itemsSold,
      gross: (item) => item.grossSales,
    },
    { key: "gross", dir: "desc" },
  );
  const counts = useMemo(
    () => ({
      active: invites.filter((item) => repStatus(item) === "active").length,
      inactive: invites.filter((item) => repStatus(item) === "inactive").length,
      pending: invites.filter((item) => repStatus(item) === "pending").length,
    }),
    [invites],
  );
  ...
  return (
    <div className="space-y-7">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Cadastros, territórios e desempenho da equipe.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Summary label="Representantes ativos" value={counts.active} />
        <Summary label="Representantes inativos" value={counts.inactive} />
        <Summary label="Convites pendentes" value={counts.pending} />
      </div>

      <section className="space-y-3">
        <div>
          <h3 className="font-semibold">Cadastros</h3>
          <p className="text-xs text-muted-foreground">
            Edite dados, situação e cidades atribuídas.
          </p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[820px] text-sm">
              <thead className="bg-secondary text-secondary-foreground">
                <tr>
                  <Header label="Nome" sortKey="name" sort={sortedInvites} />
                  <Header label="E-mail" sortKey="email" sort={sortedInvites} />
                  <Header label="Situação" sortKey="status" sort={sortedInvites} />
                  <Header label="Cidades" sortKey="cities" sort={sortedInvites} />
                  <th className="p-3 text-right">Ações</th>
                </tr>
              </thead>
              <tbody>
                {sortedInvites.sorted.length ? (
                  sortedInvites.sorted.map((invite) => (
                    <tr key={invite.id} className="border-t">
                      <td className="p-3 font-medium">{invite.name || "—"}</td>
                      <td className="p-3">{invite.email}</td>
                      <td className="p-3">
                        <StatusBadge status={repStatus(invite)} />
                      </td>
                      <td className="p-3">{invite.cities.join(", ") || "Não definidas"}</td>
                      <td className="whitespace-nowrap p-3 text-right">
                        <Button
                          size="icon"
                          variant="ghost"
                          onClick={() => beginEdit(invite)}
                          aria-label={`Editar ${invite.name ?? invite.email}`}
                        >
                        ...
                        <Button
                          ...
                          aria-label={`Remover ${invite.name ?? invite.email}`}
                        >
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Resultados de vendas e expediente no período selecionado.
          </p>
        </div>
        <Card className="space-y-4 p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
            ...
            <div className="flex items-end">
              <Button className="w-full sm:w-auto" onClick={searchPerformance}>
        ...
        <Card className="overflow-hidden p-0">
          {!searched ? (
            <div className="p-10 text-center text-sm text-muted-foreground">
              Selecione o período e clique em Buscar para carregar o desempenho.
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1050px] text-sm">
                <thead className="bg-secondary text-secondary-foreground">
                  <tr>
                    <Header label="Nome" sortKey="name" sort={sortedPerformance} />
                    <Header label="E-mail" sortKey="email" sort={sortedPerformance} />
                    <Header label="Cidades" sortKey="cities" sort={sortedPerformance} />
                    <Header label="Status" sortKey="status" sort={sortedPerformance} />
                    <Header
                      label="Horas Trabalhadas"
                      sortKey="hours"
                      sort={sortedPerformance}
                      right
                    />
                    <Header
                ...
                <tbody>
                  {sortedPerformance.sorted.map((item) => (
                    <tr key={item.id} className="border-t">
                      <td className="p-3 font-medium">{item.name || "—"}</td>
                      <td className="p-3">{item.email}</td>
                      <td className="p-3">{item.cities.join(", ") || "—"}</td>
                      <td className="p-3">
                        <StatusBadge status={item.repStatus} />
                      </td>
                      <td className="p-3 text-right">
                        {item.hours.toLocaleString("pt-BR", {
                      ...
                      <td className="p-3 text-right font-semibold">{fmtBRL(item.grossSales)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      >
        <DialogContent>
          ...
          />
          <DialogFooter>
            <Button onClick={saveRepresentative}>Salvar alterações</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function StatusBadge({ status }: { status: RepStatus }) {
  const Icon = status === "active" ? UserCheck : status === "inactive" ? UserX : Clock;
  return (
    <Badge
      variant={
...
function Header<T>({
  ...
  return (
    <th className={`p-3 ${right ? "text-right" : "text-left"}`}>
      <SortHeader
        label={label}
        sortKey={sortKey}
