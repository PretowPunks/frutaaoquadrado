...
export const Route = createFileRoute("/_app/transferencias")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Consulte pedidos, atualize entregas e gere romaneios de separação.",
      },
      { property: "og:title", content: "Pedidos & Entregas — Fruta²" },
      {
        property: "og:description",
        content: "Gestão de pedidos, entregas, pagamentos e listas de separação da Fruta².",
      },
...
function OrdersDeliveriesPage() {
  ...
  const representativeLabel = (id: string) =>
    representatives.find((rep) => rep.user_id === id)?.label ?? "Representante";
  ...
  const searchOrders = async () => {
    ...
    let query = supabase
      .from("sales")
      .select("*, products(name), customers(*)")
    ...
    setOrders(
      Array.from(grouped, ([id, items]) => {
        const first = items[0];
        const paymentStatus: PaymentStatus =
          first.payment_status === "paid" || first.status === "paid"
            ? "paid"
            : first.payment_status === "boleto" || first.payment_method === "boleto"
              ? "boleto"
              : "unpaid";
        return {
          id,
  ...
  const updateDelivery = (order: Order, status: DeliveryStatus) =>
    updateOrder(
      order,
      { order_status: status },
      `Entrega atualizada para ${deliveryLabels[status]}`,
    );
  ...
  const reportOrders = useMemo<PickingListOrder[]>(
    () =>
      orders.map((order) => ({
        id: order.id,
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Consulte a programação, atualize situações e prepare as entregas.
        </p>
      </div>
      ...
      <Card className="space-y-4 p-5">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold">Filtros da consulta</h3>
        </div>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
          ...
          >
            ...
            Gerar Romaneio / Lista de Separação
          </Button>
      ...
      <Card className="overflow-hidden p-0">
        {!searched ? (
          <div className="p-10 text-center text-sm text-muted-foreground">
            Defina os filtros e clique em Buscar para carregar os pedidos.
          </div>
        ) : orders.length === 0 ? (
          <div className="p-10 text-center text-sm text-muted-foreground">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1050px] text-sm">
              <thead className="bg-secondary text-secondary-foreground">
                <tr>
                  <th className="p-3 text-left">Data Agendada</th>
                  <th className="p-3 text-left">Cliente</th>
                  <th className="p-3 text-left">Representante</th>
                  <th className="p-3 text-left">Itens</th>
                  <th className="p-3 text-right">Valor Total</th>
                  <th className="p-3 text-center">Status da Entrega</th>
                  <th className="p-3 text-center">Status do Pagamento</th>
                </tr>
              </thead>
              <tbody>
                {orders.map((order) => (
                  <tr key={order.id} className="border-t align-top">
                    ...
                    <td className="p-3 text-center">
                      <StatusMenu
                        label={deliveryLabels[order.deliveryStatus]}
                        variant={
                          order.deliveryStatus === "delivered"
                            ? "default"
                        ...
                        options={(Object.keys(deliveryLabels) as DeliveryStatus[]).map((value) => ({
                          value,
                          label: deliveryLabels[value],
                        }))}
                        onChange={(value) => updateDelivery(order, value as DeliveryStatus)}
                      />
                    </td>
                    <td className="p-3 text-center">
                      <StatusMenu
                        ...
                        variant={
                          order.paymentStatus === "paid"
                            ? "default"
                            : order.paymentStatus === "boleto"
                              ? "secondary"
                              : "outline"
                        }
                        options={(Object.keys(paymentLabels) as PaymentStatus[]).map((value) => ({
                          value,
                          label: paymentLabels[value],
                        }))}
                        onChange={(value) => updatePayment(order, value as PaymentStatus)}
                      />
                    </td>
                  </tr>
                ))}
      ...
      <Dialog open={reportOpen} onOpenChange={setReportOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>Romaneio / Lista de Separação</DialogTitle>
          </DialogHeader>
          <div className="max-h-[68vh] overflow-auto">
            <PickingListDoc orders={reportOrders} />
          </div>
          <PrintPortal>
            <PickingListDoc orders={reportOrders} />
          </PrintPortal>
