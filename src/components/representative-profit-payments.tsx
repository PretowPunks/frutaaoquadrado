...
import {
  RepresentativeTransferReceiptDoc,
  type RepresentativeTransferReceipt,
} from "@/components/print-docs";
...
function writeLocalTransfers(records: TransferRecord[]) {
  if (typeof window !== "undefined")
    window.localStorage.setItem(LOCAL_TRANSFERS_KEY, JSON.stringify(records));
}
...
export function RepresentativeProfitPayments() {
  ...
  const load = useCallback(async () => {
    const [
      ...
      supabase
        .from("sales")
        .select(
          "id, owner_id, quantity, unit_sale_price, unit_cost, boleto_paid_at, created_at, products(name), customers(name)",
        )
      ...
      supabase
        .from("rep_invites")
        .select("accepted_user_id, name, email")
    ...
    setLabels(repLabels);
    setSales(
      ((saleRows ?? []) as unknown as BoletoSale[]).filter((sale) =>
        Boolean(repLabels[sale.owner_id]),
      ),
    );
    ...
    for (const transfer of saved) {
      if (transfer.method === "Abatimento") {
        initialDebts[transfer.repUserId] = Math.max(
          0,
          (initialDebts[transfer.repUserId] ?? 0) - transfer.profitAmount,
        );
      }
    }
  ...
  const paidSaleIds = useMemo(
    () =>
      new Set([
        ...payments.map((payment) => payment.sale_id),
        ...localTransfers.map((payment) => payment.saleId),
      ]),
    [payments, localTransfers],
  );
  ...
  const availableManualByRep = useMemo(() => {
    ...
    for (const record of localTransfers) {
      if (record.method === "Abatimento") {
        available[record.repUserId] =
          (available[record.repUserId] ?? 0) +
          Math.max(Number(record.saleProfit ?? record.profitAmount) - record.profitAmount, 0);
      } else {
  ...
  const filtered = pending.filter((sale) => {
    ...
    return (
      !term ||
      [labels[sale.owner_id], sale.products?.name, sale.customers?.name].some((value) =>
        String(value ?? "")
          .toLowerCase()
          .includes(term),
      )
  ...
  const prepareSaleTransfer = () => {
    ...
    setPendingTransfer({
      sale: paying,
      method: discount > 0 ? "Abatimento" : "PIX",
      amount: discount || remainder,
      debtBefore: debt,
    });
  };
  ...
  const confirmTransfer = () => {
    ...
    const record: TransferRecord = {
      ...
      date: paidAt,
      representativeName: labels[sale.owner_id] ?? "Representante",
      repUserId: sale.owner_id,
      saleId: sale.id,
      profitAmount: amount,
      method,
      updatedDebt,
  ...
  const prepareManualTransfer = () => {
    ...
    if (!representativeName || amount <= 0)
      return toast.error("Informe o representante e um valor válido.");
    const available = Math.max(availableManualByRep[manualRepId] ?? 0, 0);
    if (amount > available)
      return toast.error(`O valor máximo disponível para repasse manual é ${fmtBRL(available)}.`);
    const virtualSale: BoletoSale = {
      id: `manual-${Date.now()}`,
      owner_id: manualRepId,
      quantity: 1,
    ...
    setPendingTransfer({
      ...
      method: manualMethod,
      amount,
      debtBefore: debts[manualRepId] ?? 0,
    });
    setManualOpen(false);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Boletos recebidos pela Matriz geram abatimento da dívida e, quando necessário, pagamento
            manual.
          </p>
        </div>
        <Button onClick={() => setManualOpen(true)}>
          <Banknote className="h-4 w-4 mr-2" /> Registrar Repasse Manual
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-5">
          ...
          <p className="text-2xl font-bold text-destructive">{fmtBRL(pendingTotal)}</p>
        </Card>
        <Card className="p-5">
          <p className="text-xs text-muted-foreground">Lucro processado</p>
          <p className="text-2xl font-bold text-primary">{fmtBRL(paidTotal)}</p>
        </Card>
        <Card className="p-5">
          <p className="text-xs text-muted-foreground">Boletos aguardando repasse</p>
        ...
        <div className="p-4 border-b flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <Input
              ...
              placeholder="Buscar representante, cliente ou produto"
            />
          </div>
        </div>
        {filtered.length === 0 ? (
          <p className="p-5 text-sm text-muted-foreground">Nenhum lucro pendente de repasse.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-secondary text-secondary-foreground">
                <tr>
                  <th className="text-left p-3">Representante</th>
                ...
                {filtered.map((sale) => {
                  ...
                  return (
                    <tr key={sale.id} className="border-t">
                      <td className="p-3 font-medium">{labels[sale.owner_id]}</td>
                      <td className="p-3">
                        {sale.products?.name ?? "—"} · {sale.quantity} un.
                        <p className="text-xs text-muted-foreground">
                          {sale.customers?.name ?? "Sem cliente"}
                        </p>
                      </td>
                      <td className="p-3 text-right">{fmtBRL(gross)}</td>
                      <td className="p-3 text-right">{fmtBRL(cost)}</td>
                      <td className="p-3 text-right font-bold text-primary">
                        {fmtBRL(gross - cost)}
                      </td>
                      <td className="p-3 text-right">
                        <Button size="sm" onClick={() => setPaying(sale)}>
                          <Banknote className="h-4 w-4 mr-1" /> Processar lucro
                        </Button>
                      </td>
                    </tr>
                  );
      ...
      <Card className="p-0 overflow-hidden">
        <div className="p-4 border-b">
          <h3 className="font-semibold">Histórico de repasses de lucro</h3>
        </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-secondary text-secondary-foreground">
                <tr>
                  ...
                  <th className="text-right p-3">Lucro</th>
                  <th className="text-right p-3">Saldo atualizado</th>
                  <th className="p-3" />
                </tr>
              </thead>
              <tbody>
                {localTransfers.map((record) => (
                  <tr key={record.id} className="border-t">
                    ...
                    <td className="p-3">{record.representativeName}</td>
                    <td className="p-3">{record.method}</td>
                    <td className="p-3 text-right font-bold text-primary">
                      {fmtBRL(record.profitAmount)}
                    </td>
                    <td className="p-3 text-right">{fmtBRL(record.updatedDebt)}</td>
                    <td className="p-3 text-right">
                      <Button
                        size="icon"
                        variant="ghost"
      ...
      {paying && (
        <Card className="p-5 space-y-4 border-primary">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
            <div>
              ...
              <p className="font-bold">
                {fmtBRL(
                  (Number(paying.unit_sale_price) - Number(paying.unit_cost)) * paying.quantity,
                )}
              </p>
            </div>
            <div>
              <p className="text-muted-foreground">Saldo devedor</p>
              <p className="font-bold">{fmtBRL(debts[paying.owner_id] ?? 0)}</p>
            </div>
            <div>
              <p className="text-muted-foreground">Excedente manual</p>
              <p className="font-bold">
                {fmtBRL(
                  Math.max(
                    (Number(paying.unit_sale_price) - Number(paying.unit_cost)) * paying.quantity -
                      (debts[paying.owner_id] ?? 0),
                    0,
                  ),
                )}
              </p>
          ...
          <div className="flex gap-2">
            <Button onClick={prepareSaleTransfer}>Continuar para confirmação</Button>
      ...
      <Dialog open={manualOpen} onOpenChange={setManualOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Registrar Repasse Manual</DialogTitle>
            <DialogDescription>
              Use PIX ou Dinheiro somente para o valor que não pôde ser abatido do saldo devedor.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              <Label>Representante</Label>
              <Select value={manualRepId} onValueChange={setManualRepId}>
                <SelectTrigger>
              ...
              {manualRepId && (
                <p className="mt-1 text-xs text-muted-foreground">
                  Disponível para PIX/Dinheiro:{" "}
                  {fmtBRL(Math.max(availableManualByRep[manualRepId] ?? 0, 0))}
                </p>
              )}
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select
                  value={manualMethod}
                  onValueChange={(value: "PIX" | "Dinheiro") => setManualMethod(value)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button onClick={prepareManualTransfer}>Continuar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Revise os valores. A operação só será concluída após esta confirmação da Matriz.
            </DialogDescription>
          </DialogHeader>
          {pendingTransfer && (
            <div className="space-y-2 rounded-md border p-4 text-sm">
              <div className="flex justify-between">
                ...
                <strong>{labels[pendingTransfer.sale.owner_id]}</strong>
              </div>
              <div className="flex justify-between">
                <span>Forma</span>
                <strong>{pendingTransfer.method}</strong>
              </div>
              <div className="flex justify-between">
                <span>Valor</span>
                <strong>{fmtBRL(pendingTransfer.amount)}</strong>
              </div>
              <div className="flex justify-between border-t pt-2">
                <span>Saldo após confirmação</span>
                <strong>
                  {fmtBRL(
                    pendingTransfer.method === "Abatimento"
                      ? Math.max(0, pendingTransfer.debtBefore - pendingTransfer.amount)
                      : pendingTransfer.debtBefore,
                  )}
                </strong>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={confirmTransfer}>Confirmar repasse</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={Boolean(receipt)} onOpenChange={(open) => !open && setReceipt(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Relatório de Conferência</DialogTitle>
            <DialogDescription>Comprovante do repasse confirmado pela Matriz.</DialogDescription>
          </DialogHeader>
          {receipt && <RepresentativeTransferReceiptDoc receipt={receipt} />}
          <DialogFooter>
            <Button variant="outline" onClick={() => setReceipt(null)}>
              Fechar
            </Button>
            {receipt?.method === "Abatimento" &&
              Number(receipt.saleProfit ?? 0) > receipt.profitAmount && (
                <Button
                  variant="outline"
                  onClick={() => {
                    setReceipt(null);
            ...
            <Button onClick={() => window.print()}>
              <Printer className="h-4 w-4 mr-2" /> Baixar comprovante
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      {receipt && (
        <PrintPortal>
          <RepresentativeTransferReceiptDoc receipt={receipt} />
        </PrintPortal>
      )}
    </div>
  );
